'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.assertContext = exports.buildContext = exports.getCommand = undefined;

var _yargs = require('yargs');

var _yargs2 = _interopRequireDefault(_yargs);

var _context = require('../context');

var _config = require('../config');

var _config2 = _interopRequireDefault(_config);

var _assertions = require('./assertions');

var _async = require('./async');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const getCommand = exports.getCommand = () => {
  const { fullCommands } = _yargs2.default.getContext();
  const cmd = fullCommands.join(' ');
  return { cmd };
};

const buildContext = exports.buildContext = async argv => {
  const {
    managementToken,
    spaceId,
    activeSpaceId,
    environmentId,
    activeEnvironmentId,
    host,
    rawProxy,
    proxy
  } = argv;

  const context = await (0, _context.getContext)();

  if (managementToken) {
    context.managementToken = managementToken;
  }

  const space = spaceId || activeSpaceId;
  if (space) {
    context.activeSpaceId = space;
  }

  const environment = environmentId || activeEnvironmentId;
  if (environment) {
    context.activeEnvironmentId = environment;
  }
  if (!context.activeEnvironmentId) {
    context.activeEnvironmentId = 'master';
  }

  if (host) {
    context.host = host;
  }
  if (!context.host) {
    context.host = 'api.contentful.com';
  }

  if (rawProxy !== undefined) {
    context.rawProxy = rawProxy;
  }
  if (proxy) {
    context.proxy = proxy;
  }

  return { context };
};

const assertContext = exports.assertContext = async ({ cmd, context }) => {
  const { noAuthNeeded, noSpaceIdNeeded } = _config2.default;

  if (cmd && !noAuthNeeded.includes(cmd)) {
    (0, _async.handleAsyncError)(_assertions.assertLoggedIn)(context);
  }
  if (cmd && !noSpaceIdNeeded.includes(cmd)) {
    (0, _async.handleAsyncError)(_assertions.assertSpaceIdProvided)(context);
  }
};